...
void GameRenderer::check_terminal_size(int width, int height) {
    if (width < GAME_WIDTH || height < GAME_HEIGHT) {
        std::cerr << "Terminal must be at least " << GAME_WIDTH << "x" << GAME_HEIGHT << " current size: " << width << "x" << height << std::endl;
        exit(EXIT_FAILURE);
    }
}
...
void GameRenderer::draw_floor() {
    ...
    for (int i = 0; i < GAME_WIDTH; i++) {
        ...
        mvhline(prev_native_pos.y, prev_native_pos.x + 1, ACS_HLINE, i - prev_native_pos.x - 1);

        // caso in cui non sei all'ultimo render ma c'e' differenza di altezza: congiungi
...
void GameRenderer::draw_vertical_line(int start_y, int end_y, int x) {
    ...
    for (int y = start_y; y != end_y; y += direction) {
        render_str((Position){.x = x, .y = y}, "x");  // disegna la linea verticale
    }
}
